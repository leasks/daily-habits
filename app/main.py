...
def create_app(test_mode: bool | None = None) -> FastAPI:
    app = FastAPI()
    app.state.test_mode = is_test_mode(test_mode)

    def require_job_secret(req: Request):
        if req.app.state.test_mode:
            return
        if not JOB_SECRET:
            return
    ...
    async def telegram_webhook(req: Request):
        ...
        if req.app.state.test_mode:
            return {"ok": True, "test_mode": True, "parsed": parsed}
    ...
    async def weekly_memory_review(req: Request):
        ...
        if req.app.state.test_mode:
            return {"ok": True, "test_mode": True, "processed_users": 0}
    ...
    async def daily_reflection_checkin(req: Request):
        require_job_secret(req)
        if req.app.state.test_mode:
            return {"ok": True, "test_mode": True, "processed_checkins": 0}
    ...
    @app.get("/")
    def root():
        return {"status": "running", "test_mode": app.state.test_mode}
